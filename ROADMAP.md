## Strategic context
Priority: medium — v0.9.0 shipped; next focus is correctness of "active" (#74), which downstream agent rituals depend on
Current phase: Active-definition unification (#74) — route `active` through the same live-process correlation as `ps`
Blocks: ai-assistant kai-eod-ritual intensity signal (active-session count is currently unreliable); agent-deck worktree-aware listing (Phase 2.5)
Blocked by: nothing
...
Subsumes Todoist `6ghcM49pXF2JhFVw` ("list/active status seems incorrect/unreliable — shared single implementation"). Unblocks the kai-eod-ritual fleet-size signal fix (Todoist `6gmX7PCQQ9M49pFC`).
...
- [ ] `stop`/`restart`/`kill` for running sessions (Todoist `6gQfxv4Rx2g4gPxP`; exec plan `012-lifecycle-store.md`)
- [ ] Model git worktrees as first-class session location (Todoist `6gQfxvPW749CRrfP`)

Informs ai-assistant resume/kill semantics for agent crons and agent-deck worktree-aware listing. Lifecycle commands write to processes (signals), not source files — invariant #7 (read-only local data) still holds for transcripts.
...
- [ ] Gemini source: drop the stub or replace with Antigravity CLI support (Todoist `6ggg7gpJ2F5f5ggw`)
- [ ] `recap` consuming `digest` output as a richer LLM source than list previews (Todoist `6gg664VP5qFqMQ7w`)
- [ ] Compare varda (Rust task-routing CLI) vs omnisess/obi fit (Todoist `6gVpQFh3xx6HXjrP`)
