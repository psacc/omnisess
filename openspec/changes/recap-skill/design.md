...
This is the PUBLIC version. The user's private local `/recap` skill may integrate Slack, Jira, and Granola. This skill deliberately excludes those integrations so it can ship as part of the public omnisess plugin without requiring additional tool credentials.
...
**Non-Goals:**
- Integrating external data sources (Slack, Jira, Granola) — those belong in the private local skill
...
The skill body explicitly notes that Slack/Jira/Granola integration is out of scope. This is a design boundary, not a limitation. Users who want enriched recaps use a private local skill that imports additional context.
